...
Do you have an ID range in the idranges file (mp-idranges.owl), in
...
Example session from view of command line:
  ...
  # [edit in Protege]
  # [save in Protege]
  git diff mp-edit.owl
  git commit -m "polished up skull" mp-edit.owl
  git pull
  git push
...
You can check on the build here:
  http://build.berkeleybop.org/job/build-mp-edit/
  
Check for errors in the report, send an email to curators if you cannot determine what the error is.

MIREOTING
---------
Sometimes you may wish to reference a class from another ontology in the context of editing MP, and the term may not yet be mireoted. You can currently pull in a new term from GO, Uberon, Chebi, CL, PATO or PR. 
...
2. Whilst editing MP, change the "Active Ontology" file in the top header to the import file that will house the new class, for example, uberon_import.owl
